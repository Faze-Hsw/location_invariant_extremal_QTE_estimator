...
def estimate_evi_causal_fraga(data, beta_n, alpha_n):
    """估计处理组与对照组的 Candal–Fraga EVI。

    data   : dict，含 Y, D, pi_estimate（与经验分位数脚本同结构）
    beta_n : 辅助分位数水平（β_n），对应上尾概率
    alpha_n: 中间分位数水平（α_n），用于构造阈值差

    返回 dict {beta_n, alpha_n, gamma_treated, gamma_control}。
    """
    ...
    mask_t = (D == 1)
    w_t = 1.0 / pi_c[mask_t]
    q1_beta = _weighted_quantile(Y[mask_t], w_t, tau_beta)
    q1_alpha = _weighted_quantile(Y[mask_t], w_t, tau_alpha)

    # 对照组 (j=0)
    mask_c = (D == 0)
    w_c = 1.0 / (1.0 - pi_c[mask_c])
    q0_beta = _weighted_quantile(Y[mask_c], w_c, tau_beta)
    ...
    if denom1 > 0 and not np.isnan(q1_beta):
        ...
        gamma_treated = float(np.sum(weights1 * log_term1) / (n * beta_n))
    ...
    if denom0 > 0 and not np.isnan(q0_beta):
        ...
        gamma_control = float(np.sum(weights0 * log_term0) / (n * beta_n))
